...
#define AIC_PWM_CH_NUM PWM_ID_MAX_NUM
#endif

static struct aic_pwm_arg g_pwm_args[AIC_PWM_CH_NUM] = {{0}};
static bool g_pwm_init_flag = 0;

static inline void pwm_writel(u32 val, int reg) { writel(val, PWM_BASE + reg); }

static inline u32 pwm_readl(int reg) { return readl(PWM_BASE + reg); }

static inline void pwm_writel_bits(u32 val, u32 mask, u32 shift, int reg) {
  writel_bits(val, mask, shift, PWM_BASE + reg);
}

static void aic_pwm_ch_info(char *buf, u32 ch, u32 en,
                            struct aic_pwm_arg *arg) {
  static const char *mode[] = {"Up", "Down", "UpDw"};
  static const char *act[] = {"-", "Low", "Hgh", "Inv"};

  snprintf(buf, 128,
           "%2d %2d %4s %11d %3d %3s %3s %3s %3s %3s %3s\n"
           "%30s %3s %3s %3s %3s %3s\n",
           ch, en & PWM_MCTL_PWM_EN(ch) ? 1 : 0, mode[arg->mode],
           arg->tb_clk_rate, arg->def_level, act[arg->action0.CBD],
           act[arg->action0.CBU], act[arg->action0.CAD], act[arg->action0.CAU],
           act[arg->action0.PRD], act[arg->action0.ZRO], act[arg->action1.CBD],
           act[arg->action1.CBU], act[arg->action1.CAD], act[arg->action1.CAU],
...
int hal_pwm_set_tb(u32 ch, int freq) {
  ...
  if ((freq > arg->clk_rate) ||
      (freq < (arg->clk_rate / (PWM_TBCTL_CLKDIV_MAX + 1)))) {
  ...
  pwm_writel_bits(div, PWM_TBCTL_CLKDIV_MASK, PWM_TBCTL_CLKDIV_SHIFT,
                  PWM_TBCTL(ch));
...
void hal_pwm_status_show(void) {
  ...
  printf("In PWM V%d.%02d:\n"
         ...
         ver >> 8, ver & 0xFF, pwm_readl(PWM_CTL), pwm_readl(PWM_INTCTL));
...
u32 hal_pwm_int_sts(void) { return pwm_readl(PWM_INTSTS); }

void hal_pwm_clr_int(u32 stat) { pwm_writel(stat, PWM_INTSTS); }

void hal_pwm_int_config(u32 ch, enum aic_pwm_irq_event irq_mode, u8 enable) {
  pwm_writel((enable << PWM_ETSEL_INTEN_SHIFT) |
                 (irq_mode << PWM_ETSEL_INTSEL_SHIFT),
             PWM_ETSEL(ch));
...
void hal_pwm_ch_init(u32 ch, enum aic_pwm_mode mode, u32 default_level,
  ...
  if (hal_pwm_ch_check(ch) < 0) {
    hal_log_err("ch%d is hal_pwm_ch_init   hal_pwm_ch_check failed:%d\n", ch, AIC_PWM_CH_NUM - 1);
    return;
  }                    
  arg = &g_pwm_args[ch];
...
int hal_pwm_set(u32 ch, u32 duty_ns, u32 period_ns, u32 output) {
  ...
  prd = (float)arg->tb_clk_rate / arg->freq;

  hal_log_debug("output:Freq %d.%03d Hz, prd value %d, Time-base %d Hz\n",
                (int)arg->freq, (int)((arg->freq - (int)arg->freq) * 1000), prd,
                arg->tb_clk_rate);
...
static void pwm_action_set(u32 ch, struct aic_pwm_action *act, char *name) {
  ...
  action |=
      (act->CBD << PWM_AQCTL_CBD_SHIFT) | (act->CBU << PWM_AQCTL_CBU_SHIFT) |
      (act->CAD << PWM_AQCTL_CAD_SHIFT) | (act->CAU << PWM_AQCTL_CAU_SHIFT) |
...
int hal_pwm_enable(u32 ch) {
  ...
  pwm_writel_bits(arg->mode, PWM_TBCTL_CTR_MODE_MASK, PWM_TBCTL_CTR_MODE_SHITF,
                  PWM_TBCTL(ch));
