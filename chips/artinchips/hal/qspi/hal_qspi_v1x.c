...
#if defined(CONFIG_AIC_QSPI_DRV_V11) || defined(CONFIG_AIC_QSPI_DRV_V12)
...
#if CONFIG_AIC_SUPPORT_SPI_X_WIRE_IN_BIT_MODE == 4
...
int hal_qspi_master_transfer_bit_mode(qspi_master_handle *h,
                                      struct qspi_bm_transfer *t) {
...
int hal_qspi_master_init(qspi_master_handle *h,
...
int hal_qspi_master_set_cs(qspi_master_handle *h, u32 cs_num, bool enable) {
  ...
  if (enable)
    level =
        (pol == QSPI_CS_POL_VALID_LOW) ? QSPI_CS_LEVEL_LOW : QSPI_CS_LEVEL_HIGH;
  else
    level =
        (pol == QSPI_CS_POL_VALID_LOW) ? QSPI_CS_LEVEL_HIGH : QSPI_CS_LEVEL_LOW;
...
static int qspi_master_transfer_cpu_sync(qspi_master_handle *h,
  ...
  if (t->tx_data && t->rx_data) {
    ...
    ret = qspi_fifo_write_read(base, t->tx_data, t->rx_data, t->data_len,
                               tmo_cnt);
    if (ret < 0) {
...
#ifdef CONFIG_AIC_QSPI1_DYNAMIC_DMA
    1,
#endif
#ifdef CONFIG_AIC_QSPI2_DYNAMIC_DMA
    2,
#endif
#ifdef CONFIG_AIC_QSPI3_DYNAMIC_DMA
    3,
#endif
};
...
static int qspi_txrx_dma_sync(qspi_master_handle *h, struct qspi_transfer *t,
                              u32 tmo_cnt) {
  struct qspi_master_state *qspi;
  ...
  dmacfg.direction = AIC_DMA_MEM_TO_DEV;
  ...
  dmacfg.direction = AIC_DMA_DEV_TO_MEM;
  dmacfg.slave_id = qspi->dma_cfg.port_id;
...
void hal_qspi_master_irq_handler(qspi_master_handle *h) {
  ...
  if ((sts & ISTS_BIT_TF_EMP) || (sts & ISTS_BIT_TF_RDY)) {
    u32 dolen, free_len;
    if ((qspi->work_mode == QSPI_WORK_MODE_ASYNC_TX_CPU) && qspi->async_tx &&
        qspi->async_tx_remain) {
      free_len = QSPI_FIFO_DEPTH - qspi_hw_get_tx_fifo_cnt(base);
...
static int hal_qspi_transfer_rxdelay_mode(qspi_master_handle *h,
                                          u32 delay_mode) {
  struct qspi_master_state *qspi;
