...
static int qspi_txrx_dma_sync(qspi_master_handle *h, struct qspi_transfer *t,
  ...
  ret = hal_dma_prep_mode_device(qspi->dma_tx, PTR2U32(QSPI_REG_TXD(base)),
                                 ...
                                 AIC_DMA_MEM_TO_DEV, TYPE_IO_FAST);
  if (ret < 0) {
  ...
  if (!(t->data_len % HAL_QSPI_DMA_4BYTES_LINE)) {
    ...
    qspi_hw_set_fifo_watermark(base, QSPI_HALF_FIFO_DEPTH,
                               QSPI_HALF_FIFO_DEPTH);
  ...
  ret = hal_dma_prep_mode_device(qspi->dma_rx, PTR2U32(t->rx_data),
                                 ...
                                 AIC_DMA_DEV_TO_MEM, TYPE_IO_FAST);
  if (ret < 0) {
