...
#define USART_NULL_PARAM_CHK(para)                                             \
  HANDLE_PARAM_CHK(para, ERR_USART(DRV_ERROR_PARAMETER))

typedef struct {
  union {
    __IM uint32_t RBR;  /* Offset: 0x000 (R/ )  Receive buffer register */
    __OM uint32_t THR;  /* Offset: 0x000 ( /W)  Transmission hold register */
    __IOM uint32_t DLL; /* Offset: 0x000 (R/W)  Clock frequency division low
                           section register */
  };
  union {
    __IOM uint32_t DLH; /* Offset: 0x004 (R/W)  Clock frequency division high
                           section register */
  ...
  __IOM uint32_t DBG_DLL; /* Offset: 0x0B0 (R/W)  UART Clock frequency division
                             low section debug register */
  __IOM uint32_t DBG_DLH; /* Offset: 0x0B4 (R/W)  UART Clock frequency division
                             high section debug register */
} aic_usart_reg_t;
...
typedef struct {
  ...
  uint32_t RESERVED0;
  __IOM uint32_t
      RS485CTL; /* Offset: 0x0C0 (R/W ) RS485 Control and Status register*/
  __IOM uint32_t RS485AM; /* Offset: 0x0C4 (R/W ) RS485 Address Match register*/
  __IOM uint32_t
      RS485BIC; /* Offset: 0x0C8 (R/W ) RS485 Bus Idle Check register*/
} aic_usart_exreg_t;

static aic_usart_priv_t usart_instance[2];
...
static const uint32_t uart_standard_baudrate[] = {
    ...
    380400,  460800,  921600,  1000000, 1152000, 1500000,
#if defined(AIC_CHIP_D12X)
    1750000, 2000000, 2450000,
#endif
    2500000, 3000000,
};

extern int32_t drv_usart_target_init(int32_t idx, uint32_t *base, uint32_t *irq,
                                     void **handler);
...
int32_t hal_usart_config_fifo(usart_handle_t handle) {
  ...
  if (dma_flag[usart_priv->idx].dma_enable == 1) {
    ...
    addr->FCR = AIC_UART_DMA_MODE(1) | FRC_TX_FIFO_SET(3) | FRC_RX_FIFO_SET(2) |
                FCR_FIFO_EN;
  } else {
    addr->FCR =
        FCR_FIFO_EN | FCR_RX_FIFO_RST | FCR_TX_FIFO_RST | FRC_RX_FIFO_SET(3);
  }
...
int32_t hal_usart_config_baudrate(usart_handle_t handle, uint32_t baud) {
  ...
  uint32_t divisor =
      ((hal_clk_get_freq(CLK_UART0 + usart_priv->idx) * 10) / baud) >> 4;
...
/**
  ...
  \return      error code
*/
int32_t hal_usart_config_stopbits(usart_handle_t handle,
                                  usart_stop_bits_e stopbit) {
  USART_NULL_PARAM_CHK(handle);
  aic_usart_priv_t *usart_priv = handle;
  aic_usart_reg_t *addr = (aic_usart_reg_t *)(usart_priv->base);
...
/**
  ...
  \return      error code
*/
int32_t hal_usart_config_databits(usart_handle_t handle,
...
int32_t hal_usart_config_func(usart_handle_t handle, usart_func_e func) {
  ...
  aic_usart_priv_t *usart_priv = handle;
  aic_usart_reg_t *addr = (aic_usart_reg_t *)(usart_priv->base);
  aic_usart_exreg_t *exaddr =
      (aic_usart_exreg_t *)(usart_priv->base + AIC_UART_EXREG);
...
int32_t hal_usart_halt_tx_enable(usart_handle_t handle,
                                 uint8_t halt_tx_enable) {
  USART_NULL_PARAM_CHK(handle);
  aic_usart_priv_t *usart_priv = handle;
  aic_usart_reg_t *addr = (aic_usart_reg_t *)(usart_priv->base);
...
/**
  \brief       interrupt service function for transmitter holding register
  empty. \param[in]   usart_priv usart private to operate.
*/
...
*/
static void hal_usart_intr_recv_data(int32_t idx,
                                     aic_usart_priv_t *usart_priv) {
  aic_usart_reg_t *addr = (aic_usart_reg_t *)(usart_priv->base);
...
void hal_usart_intr_recv_line(int32_t idx, aic_usart_priv_t *usart_priv) {
   ...
   * error. This occurs if a new data character was received before the previous
   * data was read.
   */
...
/**
  ...
  \param[in]   usart_priv usart private to operate.
*/
static void hal_usart_intr_char_timeout(int32_t idx,
                                        aic_usart_priv_t *usart_priv) {
...
void hal_usart_irqhandler(int32_t idx) {
  ...
  case AIC_IIR_RECV_DATA: /* interrupt source:receiver data available or
...
/**
  \brief       Initialize USART Interface. 1. Initializes the resources needed
  for the USART interface 2.registers event callback function \param[in]   idx
  usart index \param[in]   cb_event  Pointer to \ref usart_event_cb_t \return
  return usart handle if success
*/
usart_handle_t hal_usart_initialize(int32_t idx, usart_event_cb_t cb_event,
                                    void *handler) {
  aic_usart_priv_t *usart_priv = &usart_instance[idx];
  usart_priv->base = UARTx_BASE(idx);
  usart_priv->irq = UARTx_IRQn(idx);
...
/**
  \brief       De-initialize UART Interface. stops operation and releases the
  software resources used by the interface \param[in]   handle  usart handle to
...
/**
  ...
  \return      error code
*/
int32_t hal_usart_config(usart_handle_t handle, uint32_t baud,
                         usart_mode_e mode, usart_parity_e parity,
                         usart_stop_bits_e stopbits, usart_data_bits_e bits,
                         usart_func_e func) {
  int32_t ret;
...
/**
  \brief       Start sending data to UART transmitter,(received data is
  ignored). The function is non-blocking,UART_EVENT_TRANSFER_COMPLETE is
  signaled when transfer completes. hal_usart_get_status can indicates if
  transmission is still in progress or pending \param[in]   handle  usart handle
  to operate. \param[in]   data  Pointer to buffer with data to send to UART
  transmitter. data_type is : uint8_t for 1..8 data bits, uint16_t for 9..16
  data bits,uint32_t for 17..32 data bits, \param[in]   num   Number of data
  items to send \return      error code
*/
...
/**
  \brief       Start receiving data from UART receiver.transmits the default
  value as specified by hal_usart_set_default_tx_value \param[in]   handle usart
  handle to operate. \param[out]  data  Pointer to buffer for data to receive
  from UART receiver \param[in]   num   Number of data items to receive \return
  error code
*/
...
*/
int32_t hal_usart_receive_query(usart_handle_t handle, void *data,
                                uint32_t num) {
  USART_NULL_PARAM_CHK(handle);
  USART_NULL_PARAM_CHK(data);
...
/**
  ...
  \param[in]   handle  usart handle to operate.
  \param[in]   data_out  Pointer to buffer with data to send to USART
  transmitter \param[out]  data_in   Pointer to buffer for data to receive from
  USART receiver \param[in]   num       Number of data items to transfer \return
  error code
*/
int32_t hal_usart_transfer(usart_handle_t handle, const void *data_out,
...
/**
  ...
  \param[in]   handle  usart handle to operate.
  \param[in]   1- Enable continuous Break transmission,0 - disable continuous
  Break transmission \return      error code
*/
...
/**
  ...
  \return      error code
*/
int32_t hal_usart_set_interrupt(usart_handle_t handle, usart_intr_type_e type,
                                int32_t flag) {
  USART_NULL_PARAM_CHK(handle);
...
/**
  ...
  \return      error code
*/
int32_t hal_usart_config_clock(usart_handle_t handle, usart_cpol_e cpol,
                               usart_cpha_e cpha) {
  USART_NULL_PARAM_CHK(handle);
  return ERR_USART(DRV_ERROR_UNSUPPORTED);
}
...
int32_t hal_uart_rx_dma_config(usart_handle_t handle, uint8_t *buf,
                               uint32_t size) {
  USART_NULL_PARAM_CHK(handle);
  ...
  config.direction = AIC_DMA_DEV_TO_MEM;
  ...
  /* config dma transfer */
  hal_dma_chan_register_cb(info->dma_chan, hal_uart_dma_rx_callback,
                           (void *)info);
  hal_dma_chan_prep_device(info->dma_chan, config.dst_addr, config.src_addr,
                           info->buf_size, AIC_DMA_DEV_TO_MEM);
  /* enable rx drq */
...
int32_t hal_uart_send_by_dma(usart_handle_t handle, uint8_t *buf,
  ...
  config.slave_id = DMA_ID_UART0 + usart_priv->idx;
  config.direction = AIC_DMA_MEM_TO_DEV;
  config.src_maxburst = 1;
  config.dst_maxburst = 1;
  ...
  hal_dma_chan_config(info->dma_chan, &config);
  /* config dma transfer */
  hal_dma_chan_register_cb(info->dma_chan, hal_uart_dma_tx_callback,
                           (void *)info);
  hal_dma_chan_prep_device(info->dma_chan, config.dst_addr, config.src_addr,
                           info->buf_size, AIC_DMA_MEM_TO_DEV);
  /* enable tx drq */
