...
void dsi_set_lane_polrs(void *base, u32 ln_polrs) {
  reg_set_bits(base + DSI_ANA_CFG1, DSI_DATA_LANE_POL_MASK,
               DSI_DATA_LANE_POL(ln_polrs));
}
...
void dsi_pkg_init(void *base, enum dsi_mode mode) {
  ...
  reg_write(base + DSI_CMD_MODE_CFG,
            DSI_CMD_MODE_CFG_MAX_RD_PKG_SIZE | DSI_CMD_MODE_CFG_DCS_LW |
                DSI_CMD_MODE_CFG_DCS_SR_0P | DSI_CMD_MODE_CFG_DCS_SW_1P |
                DSI_CMD_MODE_CFG_DCS_SW_0P | DSI_CMD_MODE_CFG_GEN_LW |
                DSI_CMD_MODE_CFG_GEN_SR_2P | DSI_CMD_MODE_CFG_GEN_SR_1P |
                DSI_CMD_MODE_CFG_GEN_SR_0P | DSI_CMD_MODE_CFG_GEN_SW_2P |
                DSI_CMD_MODE_CFG_GEN_SW_1P | DSI_CMD_MODE_CFG_GEN_SW_0P);
}

static void dsi_dphy_cfg(void *base, u32 code, u32 *p_wdata, u32 *p_rdata) {
  ...
  if (p_wdata && p_rdata) {
    ...
    reg_set_bits(TST2, DSI_PHY_TEST2_DIN_MASK, DSI_PHY_TEST2_DIN(*pi++));
    aic_udelay(1);
    reg_set_bit(TST1, DSI_PHY_TEST1_CLK);
    aic_udelay(1);
    *po++ =
        reg_rd_bits(TST2, DSI_PHY_TEST2_DOUT_MASK, DSI_PHY_CFG_LP11_TIME_SHIFT);
    aic_udelay(1);
    reg_clr_bit(TST1, DSI_PHY_TEST1_CLK);
    aic_udelay(1);
  }
...
static void dsi_dphy_cfg_hsfreq(void *base, ulong mclk) {
  ...
  static const struct {
    ...
    u32 value;
      ...
      {89, 0x00},   {99, 0x10},   {109, 0x20},  {129, 0x01},  {139, 0x11},
      {149, 0x21},  {169, 0x02},  {179, 0x12},  {199, 0x22},  {219, 0x03},
      {239, 0x13},  {249, 0x23},  {269, 0x04},  {299, 0x14},  {329, 0x05},
      {359, 0x15},  {399, 0x25},  {449, 0x06},  {499, 0x16},  {549, 0x07},
      {599, 0x17},  {649, 0x08},  {699, 0x18},  {749, 0x09},  {799, 0x19},
      {849, 0x29},  {899, 0x39},  {949, 0x0a},  {999, 0x1a},  {1049, 0x2a},
      {1099, 0x3a}, {1149, 0x0b}, {1199, 0x1b}, {1249, 0x2b}, {1299, 0x3b},
      {1349, 0x0c}, {1399, 0x1c}, {1449, 0x2c}, {1499, 0x3c},
  };
...
void dsi_phy_init(void *base, ulong mclk, u32 lane, enum dsi_mode mode) {
  ...
  reg_set_bits(ANA2, DSI_ANA_CFG2_EN_LDO_MASK, DSI_ANA_CFG2_EN_LDO(0x1F));
  aic_udelay(10);
  ...
  aic_udelay(5);
  reg_set_bits(CFG, DSI_PHY_CFG_LP11_TIME_MASK, DSI_PHY_CFG_LP11_TIME(10));
...
void dsi_hs_clk(void *base, u32 enable) {
  ...
  if (enable) {
    ret = readl_poll_timeout(base + DSI_PHY_STA, val,
                             val & DSI_PHY_STA_STOP_STATE_C, DSI_TIMEOUT_US);
    if (ret) {
...
void dsi_set_vm(void *base, enum dsi_mode mode, enum dsi_format format,
  ...
  reg_set_bits(base + DSI_DPI_VC, DSI_DPI_VC_NUM_MASK, DSI_DPI_VC_NUM(vc));
  ...
  reg_set_bits(VIDCFG, DSI_VID_MODE_CFG_TYPE_MASK, DSI_VID_MODE_CFG_TYPE(mode));
  ...
  reg_set_bits(base + DSI_VID_HINACT_TIME, DSI_VID_HSA_TIME_MASK,
               DSI_VID_HSA_TIME(timing->hsync_len * dsi_bits_per_pixel[format] /
                                (8 * lane)));

  reg_set_bits(base + DSI_VID_HINACT_TIME, DSI_VID_HBP_TIME_MASK,
               DSI_VID_HBP_TIME(timing->hback_porch *
                                dsi_bits_per_pixel[format] / (8 * lane)));

  reg_write(base + DSI_VID_HT_TIME,
            ht * dsi_bits_per_pixel[format] / (8 * lane));

  reg_set_bits(base + DSI_VID_VACT_LINE, DSI_VID_VSA_LINE_MASK,
               DSI_VID_VSA_LINE(timing->vsync_len));
  reg_set_bits(base + DSI_VID_VBLANK_LINE, DSI_VID_VBP_TIME_MASK,
               DSI_VID_VBP_TIME(timing->vback_porch));
  reg_set_bits(base + DSI_VID_VBLANK_LINE, DSI_VID_VFP_LINE_MASK,
               DSI_VID_VFP_LINE(timing->vfront_porch));
  reg_set_bits(base + DSI_VID_VACT_LINE, DSI_VID_VACT_TIME_MASK,
...
void dsi_cmd_wr(void *base, u32 dt, u32 vc, const u8 *data, u32 len) {
  ...
  ret = readl_poll_timeout(base + DSI_PHY_STA, val,
                           val & DSI_PHY_STA_STOP_STATE_0, DSI_TIMEOUT_US);
  if (ret) {
    pr_err("Timeout during wait phy stop state 0\n");
    return;
  }

  reg_write(base + DSI_GEN_PH_CFG, (d1 << 16) | (d0 << 8) | (vc << 6) | dt);

  ret = readl_poll_timeout(base + DSI_CMD_PKG_STA, val,
                           val & DSI_CMD_PKG_STA_PLD_W_EMPTY, DSI_TIMEOUT_US);
  if (ret) {
