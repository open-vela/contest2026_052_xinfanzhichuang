...
void i8080_cmd_ctl(void *base, u32 first_line, u32 other_line) {
  reg_set_bits(base + DBI_I8080_COMMAND_CTL, FIRST_LINE_COMMAND_MASK,
               FIRST_LINE_COMMAND(first_line));
  reg_set_bit(base + DBI_I8080_COMMAND_CTL, FIRST_LINE_COMMAND_CTL);

  reg_set_bits(base + DBI_I8080_COMMAND_CTL, OTHER_LINE_COMMAND_MASK,
...
u32 i8080_rd_data(void *base) { return reg_read(base + DBI_I8080_RD_DATA); }

u32 i8080_rd_fifo_depth(void *base) {
  return reg_rd_bits(base + DBI_I8080_FIFO_DEPTH, DBI_I8080_RD_FIFO_DEPTH_MASK,
                     DBI_I8080_RD_FIFO_DEPTH_SHIFT);
}

u32 i8080_wr_fifo_depth(void *base) {
  return reg_rd_bits(base + DBI_I8080_FIFO_DEPTH, DBI_I8080_WR_FIFO_DEPTH_MASK,
...
void qspi_mode_cfg(void *base, u32 code1_cfg, u32 vbp_num, u32 qspi_mode) {
  reg_set_bits(base + DBI_QSPI_MODE, DBI_SPI_CODE1_CFG_MASK,
               DBI_SPI_CODE1_CFG(code1_cfg));

  reg_set_bits(base + DBI_QSPI_MODE, DBI_SPI_VBP_NUM_MASK,
...
void spi_cmd_ctl(void *base, u32 first_line, u32 other_line) {
  reg_set_bits(base + DBI_SPI_COMMAND_CTL, FIRST_LINE_COMMAND_MASK,
               FIRST_LINE_COMMAND(first_line));
  reg_set_bit(base + DBI_SPI_COMMAND_CTL, FIRST_LINE_COMMAND_CTL);

  reg_set_bits(base + DBI_SPI_COMMAND_CTL, OTHER_LINE_COMMAND_MASK,
               OTHER_LINE_COMMAND(other_line));
...
void spi_wr_cmd(void *base, u32 cmd) { reg_write(base + DBI_SPI_WR_CMD, cmd); }
...
u32 spi_rd_data(void *base) { return reg_read(base + DBI_SPI_RD_DATA); }

u32 spi_rd_fifo_depth(void *base) {
  return reg_rd_bits(base + DBI_SPI_FIFO_DEPTH, DBI_SPI_RD_FIFO_DEPTH_MASK,
                     DBI_SPI_RD_FIFO_DEPTH_SHIFT);
}

u32 spi_wr_fifo_depth(void *base) {
  return reg_rd_bits(base + DBI_SPI_FIFO_DEPTH, DBI_SPI_WR_FIFO_DEPTH_MASK,
...
void spi_cmd_wr(void *base, u32 code, u32 count, const u8 *data) {
  ...
  ret = readl_poll_timeout(
      base + DBI_SPI_STATUS, val,
      ((val & (DBI_SPI_IDEL | DBI_SPI_TX_FIFO_EMPTY)) == 0x2), DBI_TIMEOUT_US);
  if (ret)
