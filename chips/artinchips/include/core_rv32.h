...
#define CLIC_CLICCFG_NVBIT_Msk                                                 \
  (0x1UL << CLIC_CLICCFG_NVBIT_Pos) /* NVBIT Mask */

#define CLIC_CLICCFG_NLBIT_Pos 1U /* NLBIT Position */
#define CLIC_CLICCFG_NLBIT_Msk                                                 \
  (0xFUL << CLIC_CLICCFG_NLBIT_Pos) /* NLBIT Mask */

#define CLIC_CLICCFG_NMBIT_Pos 5U /* NMBIT Position */
#define CLIC_CLICCFG_NMBIT_Msk                                                 \
  (0x3UL << CLIC_CLICCFG_NMBIT_Pos) /* NMBIT Mask */
...
/**
  ...
  \details Enable a secure device-specific interrupt in the VIC interrupt
...
/**
  ...
  \details Disable a secure device-specific interrupt in the VIC interrupt
  controller. \param [in]      IRQn  External interrupt number. Value cannot be
  negative.
 */
...
/**
  ...
  \details Read the enabled register in the VIC and returns the pending bit for
  the specified interrupt. \param [in]      IRQn  Interrupt number. \return 0
  Interrupt status is not enabled. \return             1  Interrupt status is
  enabled.
 */
...
/**
  ...
  \details Read the pending register in the VIC and returns the pending bit for
  the specified interrupt. \param [in]      IRQn  Interrupt number. \return 0
  Interrupt status is not pending. \return             1  Interrupt status is
  pending.
 */
...
__STATIC_INLINE void csi_vic_set_prio(int32_t IRQn, uint32_t priority) {
  uint8_t nlbits = (CLIC->CLICINFO & CLIC_INFO_CLICINTCTLBITS_Msk) >>
                   CLIC_INFO_CLICINTCTLBITS_Pos;
  CLIC->CLICINT[IRQn].CTL =
      (CLIC->CLICINT[IRQn].CTL & (~CLIC_INTCFG_PRIO_Msk)) |
      (priority << (8 - nlbits));
}
...
__STATIC_INLINE void csi_vic_set_threshold(uint32_t threshold) {
  ...
  CLIC->MINTTHRESH =
      (threshold << CLIC_MINTTHRESH_MTH_Pos) & CLIC_MINTTHRESH_MTH_Msk;
}

__STATIC_INLINE uint32_t csi_vic_get_threshold(void) {
  uint8_t threshold =
      (CLIC->MINTTHRESH & CLIC_MINTTHRESH_MTH_Msk) >> CLIC_MINTTHRESH_MTH_Pos;
  uint8_t nlbits = (CLIC->CLICINFO & CLIC_INFO_CLICINTCTLBITS_Msk) >>
                   CLIC_INFO_CLICINTCTLBITS_Pos;
  uint8_t pos = 8 - nlbits;
...
/**
  ...
  \details Set the interrupt handler according to the interrupt num, the handler
  will be filled in irq vectors. \param [in]      IRQn  Interrupt number. \param
...
/**
  ...
  */
__STATIC_INLINE void csi_mpu_config_region(uint32_t idx, uint32_t base_addr,
                                           region_size_e size,
...
/**
  ...
  \param [in]  enable     enable or disable memory protected region.
  */
__STATIC_INLINE void csi_sysmap_config_region(uint32_t idx, uint32_t base_addr,
                                              uint32_t attr) {
  uint32_t addr = 0;
...
/**
  ...
  \details Check whether a character is pending for reading in the variable \ref
  HAD_RxBuffer. \return          0  No character available. \return          1
  Character available.
 */
...
__STATIC_INLINE void csi_dcache_enable(void) {
  ...
  if ((cache & CACHE_MHCR_DE_Msk) == 0) {
    ...
    cache |= (CACHE_MHCR_DE_Msk | CACHE_MHCR_WB_Msk | CACHE_MHCR_WA_Msk |
              CACHE_MHCR_RS_Msk | CACHE_MHCR_BPE_Msk |
              CACHE_MHCR_L0BTB_Msk); /* enable all Cache */
    __set_MHCR(cache);
  }
  mhint = __get_MHINT();
  ...
  mhint |=
      (1 << MHINT_DPLD_Pos) | (1 << MHINT_AMR_Pos) | (3 << MHINT_PREF_N_Pos);
#else
...
/**
  ...
  \param[in]   dsize   size of memory block (aligned to cache-line-size)
*/
__STATIC_INLINE void csi_dcache_clean_invalid_range(phy_addr_t addr,
                                                    u32 dsize) {
#if (__DCACHE_PRESENT == 1U)
  s32 op_size = dsize + addr % CACHE_LINE_SIZE;
  phy_addr_t op_addr = addr & CACHE_INV_ADDR_Msk;
...
/**
  ...
  */
__STATIC_INLINE void csi_cache_set_range(uint32_t index, uint32_t baseAddr,
...
/**
  ...
  */
__STATIC_INLINE uint32_t csi_cache_get_access_time(void) { return 0; }
...
/**
  ...
  */
__STATIC_INLINE uint32_t csi_cache_get_miss_time(void) { return 0; }
