...
#ifdef CONFIG_AIC_PANEL_BACKLIGHT
#define AIC_PANEL_BACKLIGHT CONFIG_AIC_PANEL_BACKLIGHT
#else
#define AIC_PANEL_BACKLIGHT "/dev/pwm0"
#endif
...
static int panel_backlight_init(void) {
  if (pwm_fd < 0) {
    pwm_fd = open(AIC_PANEL_BACKLIGHT, O_RDWR);
    if (pwm_fd < 0) {
      return -1;
    }
  }
...
#ifdef CONFIG_AIC_PM_INDEPENDENT_POWER_KEY
...
#ifndef CONFIG_AIC_PANEL_ENABLE_GPIO_LOW
  hal_gpio_clr_output(g, p);
#else
  hal_gpio_set_output(g, p);
#endif
