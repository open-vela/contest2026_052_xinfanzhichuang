...
bool is_ui_rect_win_overlap(struct aic_de_comp *comp, u32 layer_id, u32 rect_id,
...
bool is_valid_ui_rect_size(struct aic_de_comp *comp,
  ...
  /* check overlap  */
  if (is_ui_rect_win_overlap(comp, layer_data->layer_id, layer_data->rect_id,
...
bool is_all_rect_win_disabled(struct aic_de_comp *comp, u32 layer_id) {
  int i;
  u32 index;
...
int ui_rect_disable(struct aic_de_comp *comp, u32 layer_id, u32 rect_id) {
...
int config_ui_layer_rect(struct aic_de_comp *comp,
  ...
  x_offset += CONFIG_AIC_SCREEN_CROP_POS_X;
  y_offset += CONFIG_AIC_SCREEN_CROP_POS_Y;
#endif

  de_ui_layer_set_rect(comp->regs, in_w, in_h, x_offset, y_offset, stride0,
                       addr0, layer_data->rect_id);
