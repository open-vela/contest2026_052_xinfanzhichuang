...
static struct dsi_irq_info irq1_info[] = {
    {DSI_IRQ_STA1_PHY_ERR_STA_DATA0_ERRCONTENTION_LP1,
     "PHY ERROR: Data0 ErrContention LP1"},
    {DSI_IRQ_STA1_PHY_ERR_STA_DATA0_ERRCONTENTION_LP0,
     "PHY ERROR: Data0 ErrContention LP0"},
    {DSI_IRQ_STA1_PHY_ERR_STA_DATA0_ERRCONTROL,
     "PHY ERROR:DSI Data0 ErrControl"},
    {DSI_IRQ_STA1_PHY_ERR_STA_DATA0_ERRSYNCESC_LPDT,
     "PHY ERROR: Data0 ErrSyncEsc LPDT"},
    {DSI_IRQ_STA1_PHY_ERR_STA_DATA0_ERRESC_ESCAPE_ENTRY,
     "PHY ERROR: Data0 ErrEsc escape entry"},
    {DSI_IRQ_STA1_ACK_ERR_STA_DSI_PROTOCOL_VIOLATION,
     "ACK ERROR: DSI Protocol Violation"},
    {DSI_IRQ_STA1_ACK_ERR_STA_INVALID_TRANSMISSION_LENGTH,
     "ACK ERROR: Invalid Transmission Length"},
    {DSI_IRQ_STA1_ACK_ERR_STA_DSI_VC_ID_INVALID,
     "ACK ERROR: DSI VC ID Invalid"},
    {DSI_IRQ_STA1_ACK_ERR_STA_DSI_DATA_TYPE_NOT_RECOGNIZED,
     "ACK ERROR: DSI Data Type Not Recognized"},
    {DSI_IRQ_STA1_ACK_ERR_STA_LONG_PACKET_CHECKSUM_ERROR,
     "ACK ERROR: Long Packet Checksum Error"},
    {DSI_IRQ_STA1_ACK_ERR_STA_ECC_ERROR_MULTI_BIT,
     "ACK ERROR: ECC Error multi-bit"},
    {DSI_IRQ_STA1_ACK_ERR_STA_ECC_ERROR_SINGLE_BIT,
     "ACK ERROR: ECC Error single-bit"},
    {DSI_IRQ_STA1_ACK_ERR_STA_CONTENTION_DETECTED,
     "ACK ERROR: Contention Detected"},
    {DSI_IRQ_STA1_ACK_ERR_STA_FALSE_CONTROL_ERROR,
     "ACK ERROR: False Control Error"},
    {DSI_IRQ_STA1_ACK_ERR_STA_PERIPHERAL_TIMEOUT_ERROR,
     "ACK ERROR: Peripheral Timeout Error"},
    {DSI_IRQ_STA1_ACK_ERR_STA_LOW_POWER_TRANSMIT_SYNC_ERROR,
     "ACK ERROR: Low Power Transmit Sync Error"},
    {DSI_IRQ_STA1_ACK_ERR_STA_ESCAPE_MODE_ENTRY_COMMAND_ERROR,
     "ACK ERROR: Escape Mode Entry Command Error"},
...
static int aic_dsi_set_vm(const struct display_timing *timing, int enable) {
  ...
  if (enable) {
    ...
    dsi_set_vm(comp->regs, dsi->mode, dsi->format, dsi->lane_num, comp->vc_num,
               timing);
  } else {
    dsi_set_vm(comp->regs, DSI_MOD_CMD_MODE, dsi->format, dsi->lane_num,
               comp->vc_num, timing);
...
static int aic_dsi_probe(void) {
  ...
  comp = (struct aic_dsi_comp *)kmm_zalloc(sizeof(*comp));
  if (!comp) {
  ...
  irq_attach(MIPI_DSI_IRQn, (xcpt_t)aic_dsi_handler, NULL);
