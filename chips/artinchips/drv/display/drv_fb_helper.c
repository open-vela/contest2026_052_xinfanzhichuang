...
void aicfb_update_layer(struct aicfb_info *fbi) {
  ...
  case 0:
    ...
    break;
#ifdef CONFIG_AIC_FB_ROTATE_EN
  case 90:
  case 270: {
    unsigned int stride = ALIGN_8B(fbi->height * fbi->bits_per_pixel / 8);

    layer.buf.phy_addr[0] =
        (uintptr_t)fbi->fb_start + fbi->fb_size * AIC_FB_DRAW_BUF_NUM;
...
void aicfb_enable_panel(struct aicfb_info *fbi, u32 on) {
  ...
  pr_info("aicfb_enable_panel: %s\n", on == AICFB_ON ? "ON" : "OFF");

  if (on == AICFB_ON) {
    pr_info("  Panel name: %s\n", panel->name);
    pr_info("  Connector type: %d\n", panel->connector_type);
    if (panel->timings) {
      pr_info("  Timing: %dx%d, pixelclock=%u Hz (%.2f MHz)\n",
              panel->timings->hactive, panel->timings->vactive,
              panel->timings->pixelclock,
              panel->timings->pixelclock / 1000000.0);
    }
    funcs->prepare();
    pr_info("  Calling di->di_funcs->enable()...\n");
    di->di_funcs->enable();
    if (de && de->de_funcs && de->de_funcs->timing_enable) {
      pr_info("  Calling de->de_funcs->timing_enable()...\n");
      de->de_funcs->timing_enable();
    } else {
      pr_warn("  de or de_funcs or timing_enable is NULL, skipping timing "
              "enable\n");
    }
    pr_info("  Calling funcs->enable()...\n");
    funcs->enable(panel);
  } else {
    funcs->disable(panel);
    if (de && de->de_funcs && de->de_funcs->timing_disable) {
      de->de_funcs->timing_disable();
    }
    di->di_funcs->disable();
    funcs->unprepare();
    pr_info("Panel disabled successfully\n");
  }
}
...
#ifdef CONFIG_AIC_DISABLE_DITHER
  dither_en = 0;
#endif
...
static void aicfb_get_hv_active(struct aicfb_info *fbi, u32 *active_w,
                                u32 *active_h) {
#ifdef CONFIG_AIC_SCREEN_CROP
  *active_w = CONFIG_AIC_SCREEN_CROP_WIDTH;
  *active_h = CONFIG_AIC_SCREEN_CROP_HEIGHT;
#else
  struct aic_panel *panel = fbi->panel;
  struct panel_desc *desc = fbi->desc;
...
#ifdef CONFIG_AIC_FB_ROTATE_EN
  fbi->fb_rotate = CONFIG_AIC_FB_ROTATE_DEGREE;
#else
  fbi->fb_rotate = 0;
#endif
...
#ifdef CONFIG_AIC_DISP_COLOR_BLOCK
#ifdef CONFIG_AIC_FB_ROTATE_EN
  if (!fb_size)
    return;

  memset(fbi->fb_start, 0x0, fb_size);
  hal_dcache_clean_invalidate((unsigned long)fbi->fb_start, fb_size);
#else
  u32 width, height;
  u32 i, j, index;
...
#if defined(CONFIG_AICFB_ARGB8888) || defined(CONFIG_AICFB_ABGR8888) ||                      \
    defined(CONFIG_AICFB_XRGB8888)
  case 32: {
    for (i = 0; i < height; i++) {
      for (j = 0; j < width; j++) {
        index = (i / 100 + j / 100) % 5;
...
#endif
#if defined(CONFIG_AICFB_RGB888)
  case 24: {
    for (i = 0; i < height; i++) {
      for (j = 0; j < width; j++) {
        index = (i / 100 + j / 100) % 5;
...
#endif
#if defined(CONFIG_AICFB_RGB565)
  case 16: {
    for (i = 0; i < height; i++) {
      for (j = 0; j < width; j++) {
        index = (i / 100 + j / 100) % 5;
...
#ifdef CONFIG_AIC_DISP_BLACK_SCREEN
  if (!fb_size)
    return;
...
static void aicfb_reset(struct aicfb_info *fbi) {
  ...
  layer.buf.size.width =
      desc ? desc->timings->hactive : panel->timings->hactive;
  layer.buf.size.height =
      desc ? desc->timings->vactive : panel->timings->vactive;
...
static int handle_dbi_data(struct aic_panel *panel, struct panel_desc *desc,
                           struct panel_dbi *dbi, u32 *cmd_offset) {
  struct panel_dbi_commands *target =
      desc ? &desc->dbi->commands : &panel->dbi->commands;

  if (!target->buf) {
    target->buf = kmm_zalloc(target->len);
    if (!target->buf) {
  ...
  if (total < target->len) {
    memcpy((u8 *)target->buf + *cmd_offset, dbi->commands.buf,
           dbi->commands.len);
...
static int handle_dsi_data(struct aic_panel *panel, struct panel_desc *desc,
                           struct panel_dsi *dsi, u32 *cmd_offset) {
  struct dsi_command *target =
      desc ? &desc->dsi->command : &panel->dsi->command;

  if (!target->buf) {
    target->buf = kmm_zalloc(4096);
    if (!target->buf) {
  ...
  memcpy(target->buf + *cmd_offset, dsi->command.buf, dsi->command.len);
...
static int handle_spi_rgb_data(struct aic_panel *panel, struct panel_desc *desc,
                               struct panel_rgb *rgb, u32 *cmd_offset) {
  struct rgb_spi_command *target =
      desc ? &desc->rgb->spi_command : &panel->rgb->spi_command;
  ...
  memcpy(target->buf + *cmd_offset, rgb->spi_command.buf, rgb->spi_command.len);
...
static bool handle_rgb_config(struct aicfb_info *fbi,
                              struct aicfb_pq_config *config) {
  struct aic_panel *panel = fbi->panel;
  struct panel_desc *desc = fbi->desc;
...
static bool handle_mipi_config(struct aicfb_info *fbi,
                               struct aicfb_pq_config *config) {
  struct aic_panel *panel = fbi->panel;
  struct panel_desc *desc = fbi->desc;
...
#define MIPI_DSI_UART_COPY_COMMAND 4
  ...
  case MIPI_DSI_ADB_UPDATE_COMMAND: {
    if (!target_dsi->command.buf)
      target_dsi->command.buf = kmm_zalloc(4096);
  ...
  case MIPI_DSI_UART_COPY_COMMAND: {
    ...
    memcpy(target_dsi->command.buf + dsi_commands_offset, dsi->command.buf,
           dsi->command.len);
...
void aicfb_pq_set_config(struct aicfb_info *fbi,
                         struct aicfb_pq_config *config) {
  struct aic_panel *panel = fbi->panel;
  struct panel_desc *desc = fbi->desc;
...
void aicfb_pq_get_config(struct aicfb_info *fbi,
                         struct aicfb_pq_config *config) {
  struct aic_panel *panel = fbi->panel;
  struct panel_desc *desc = fbi->desc;
