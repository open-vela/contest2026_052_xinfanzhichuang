...
static int aic_rgb_clk_enable(void) {
  ...
  pr_info("Starting RGB clock enable...\n");
  pr_info("  pixclk: %u Hz (%.2f MHz)\n", pixclk, pixclk / 1000000.0);
  pr_info("  sclk_rate: %llu Hz (%.2f MHz)\n", comp->sclk_rate,
          comp->sclk_rate / 1000000.0);
  pr_info("  pll_disp_rate: %llu Hz (%.2f MHz)\n", comp->pll_disp_rate,
          comp->pll_disp_rate / 1000000.0);
  ...
  hal_clk_enable(CLK_LCD);

  hal_reset_deassert(RESET_LCD);

  pr_info("RGB clock enabled successfully\n");
  aic_rgb_release_drvdata();
  return 0;
}

static int aic_rgb_clk_disable(void) {
  hal_reset_assert(RESET_LCD);

  hal_clk_disable(CLK_LCD);
...
static void aic_rgb_swap(void) {
  ...
  if (rgb->data_mirror) {
    reg_set_bits(comp->regs + RGB_DATA_SEQ_SEL, RGB_DATA_OUT_SEL_MASK,
                 RGB_DATA_OUT_SEL(7));
...
#endif
    reg_set_bits(comp->regs + RGB_DATA_SEQ_SEL, RGB_DATA_OUT_SEL_VALID_MASK,
                 RGB_DATA_OUT_SEL_VALID(bits_valid));
  }
  ...
  if (rgb->clock_phase)
    reg_set_bits(comp->regs + RGB_CLK_CTL, CKO_PHASE_SEL_MASK,
                 CKO_PHASE_SEL(rgb->clock_phase));

  aic_rgb_release_drvdata();
}

#ifdef CONFIG_AIC_PANEL_SPI_EMULATION
...
static int aic_rgb_enable(void) {
  ...
  reg_set_bits(comp->regs + RGB_LCD_CTL, RGB_LCD_CTL_MODE_MASK,
               RGB_LCD_CTL_MODE(rgb->mode));

  switch (rgb->mode) {
  case PRGB:
    reg_set_bits(comp->regs + RGB_LCD_CTL, RGB_LCD_CTL_PRGB_MODE_MASK,
                 RGB_LCD_CTL_PRGB_MODE(rgb->format));
    aic_rgb_swap();
    pr_info("  RGB mode: PRGB, format: %d\n", rgb->format);
    break;
  case SRGB:
    if (rgb->format)
      reg_set_bit(comp->regs + RGB_LCD_CTL, RGB_LCD_CTL_SRGB_MODE);
    aic_rgb_swap();
    pr_info("  RGB mode: SRGB, format: %d\n", rgb->format);
    break;
  default:
  ...
  pr_info("RGB enabled successfully\n");
  aic_rgb_release_drvdata();
...
static int aic_rgb_probe(void) {
  ...
  comp = (struct aic_rgb_comp *)kmm_zalloc(sizeof(*comp));
  if (!comp) {
