...
struct aic_de_vi_format_info *
de_convert_vi_format(enum mpp_pixel_format format) {
...
static int de_set_video_tile_format(struct aic_de_comp *comp,
                                    struct aicfb_layer_data *layer_data) {
  ...
  case MPP_FMT_YUV420_64x32_TILE:
    ...
    if (crop_en) {
      ...
      offset_p0 = ALIGN_64B(stride0) * 32 * tile_p0_y + 64 * 32 * tile_p0_x;

      offset_p1 = ALIGN_64B(stride1) * 32 * tile_p1_y + 64 * 32 * tile_p1_x;
  ...
  case MPP_FMT_YUV420_128x16_TILE:
    ...
    if (crop_en) {
      ...
      offset_p0 = ALIGN_128B(stride0) * 16 * tile_p0_y + 128 * 16 * tile_p0_x;

      offset_p1 = ALIGN_128B(stride1) * 16 * tile_p1_y + 128 * 16 * tile_p1_x;
  ...
  de_set_video_layer_info(comp->regs, in_w, in_h, format, stride0, stride1,
                          addr0, addr1, addr2, x_offset, y_offset);

  de_set_video_layer_tile_offset(comp->regs, tile_p0_x_offset, tile_p0_y_offset,
  ...
  de_set_scaler0_channel(comp->regs, in_w, in_h, scaler_w, scaler_h, 0);

  de_set_scaler0_channel(comp->regs, in_w_ch1, in_h_ch1, scaler_w, scaler_h, 1);

  de_check_scaler0_active(comp, in_w_ch1, in_h_ch1, scaler_w, scaler_h);
...
static int de_set_video_rgb_format(struct aic_de_comp *comp,
                                   struct aicfb_layer_data *layer_data) {
  enum mpp_pixel_format format = layer_data->buf.format;
  u32 in_w = (u32)layer_data->buf.size.width;
  u32 in_h = (u32)layer_data->buf.size.height;
...
static int de_set_video_planar_format(struct aic_de_comp *comp,
                                      struct aicfb_layer_data *layer_data) {
  enum mpp_pixel_format format = layer_data->buf.format;
  u32 in_w = (u32)layer_data->buf.size.width;
  u32 in_h = (u32)layer_data->buf.size.height;
  ...
  case MPP_FMT_YUV420P:
    ...
    if (crop_en) {
      ...
      ch1_offset = (crop_x >> 1) + (crop_y >> 1) * stride1;
  ...
  if (scaler_en) {
    ...
    if (channel_num == 2) {
      de_set_scaler0_channel(comp->regs, in_w_ch1, in_h_ch1, scaler_w, scaler_h,
                             1);
...
static int de_set_video_packed_format(struct aic_de_comp *comp,
                                      struct aicfb_layer_data *layer_data) {
  enum mpp_pixel_format format = layer_data->buf.format;
  u32 in_w = (u32)layer_data->buf.size.width;
  u32 in_h = (u32)layer_data->buf.size.height;
...
static void de_vi_layer_config_crop_pos(struct aic_de_comp *comp,
  ...
  u32 x_offset = layer_data->pos.x + CONFIG_AIC_SCREEN_CROP_POS_X;
  u32 y_offset = layer_data->pos.y + CONFIG_AIC_SCREEN_CROP_POS_Y;
